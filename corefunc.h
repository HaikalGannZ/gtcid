		...
		if (packet)
		{
			...
			if (extended)
			{
				...
				else if (func == "onShowCaptcha")
				{
					...
					if (ctx.find("add_label_with_icon|big|`wAre you Human?``|left|206|") != std::string::npos)
						fixCaptcha(ctx);
				}
				...
				else if (func == "OnSpawn")
				{
					...
					if (ctx.find("type|local") != std::string::npos)
					{
						...
						localid = netid;
						localX = atoi(gg[0].c_str());
						localY = atoi(gg[1].c_str());
	...
	void ProcessTankUpdatePacket(float someVal, EntityComponent *entityComponent, BYTE *structPointer, ENetPacket *packets)
	{
		...
		case 4:
		{
			...
			for (int i = 0; i < world->tileCount; i++)
			{
				...
				else if (world->foreground[i] == 546)
					worldPtr += (*(__int16_t *)(worldPtr + 5)) + 4;
				else if (world->foreground[i] == 12)
					worldPtr += (*(__int16_t *)(worldPtr + 5)) + 4;
				else if (world->foreground[i] == 3808)
					worldPtr += 1;
			...
			if (world->name.find("TUTORIAL_1") != std::string::npos)
				SendPacket(2, "action|growid", peer);
		...
		case 0: // AvatarPacketReceiver::LerpState
		{
			...
			{
				if (datak->punchX != -1 && datak->punchY != -1)
					sendPunch(datak->punchX, datak->punchY, datak->x, datak->y, datak->netID);
				SendPacketRaw(4, packPlayerMoving(datak), 56, 0, peer, ENET_PACKET_FLAG_RELIABLE);
			}
